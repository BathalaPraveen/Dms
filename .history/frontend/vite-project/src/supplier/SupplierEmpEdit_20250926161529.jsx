...
//   const { supplierIndex, supplierIndex } = useParams();
  ...
  const validate = () => {
    ...
    if (!formData.country)
      newErrors.country = t("supplier.required", { field: t("employee.country") });
    if (!formData.state)
  ...
  console.log("Supplier Index:", supplierIndex, "Employee Index:", supplierIndex);
    if (suppliers[supplierIndex] && suppliers[supplierIndex].empList) {
      const emp = suppliers[supplierIndex].empList[supplierIndex];
      if (emp) {
        setFormData(emp); // load into state
      }
    }
  ...
  const handleUpdate = () => {
    ...
    if (!suppliers[supplierIndex].empList) suppliers[supplierIndex].empList = [];

    suppliers[supplierIndex].empList[supplierIndex] = formData;
  ...
  const handleCancel = () => {
    navigate(`/supplier/supplieremp/${supplierIndex}`);
  };
    ...
    <div className="container mt-3">
      ...
      >
        <div className="card-body d-flex justify-content-between align-items-center">
          <h4 className="card-title mb-0">{t("employee.editemployeetitle")}</h4>
