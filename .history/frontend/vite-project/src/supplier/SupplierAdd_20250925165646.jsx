...
const SupplierAdd = () => {
  ...
  const validate = () => {
    const newErrors = {};
    // User Type & Zone
    if (!formData.userType) newErrors.userType = t("supplier.userTypeRequired");
    if (!formData.zone) newErrors.zone = t("supplier.zoneRequired");

    // Employee ID
    if (!formData.employeeId) {
      newErrors.employeeId = t("supplier.employeeIdRequired");
    } else if (!/^[A-Za-z0-9]+$/.test(formData.employeeId)) {
      newErrors.employeeId = t("supplier.employeeIdInvalid");
    }

    // First Name
    if (!formData.firstName) {
      newErrors.firstName = t("supplier.firstNameRequired");
    } else if (!/^[A-Za-z][A-Za-z\s]*$/.test(formData.firstName.trim())) {
      newErrors.firstName = t("supplier.firstNameInvalid");
    }

    // Last Name
    if (!formData.lastName) {
      newErrors.lastName = t("supplier.lastNameRequired");
    } else if (!/^[A-Za-z][A-Za-z\s]*$/.test(formData.lastName.trim())) {
      newErrors.lastName = t("supplier.lastNameInvalid");
    }

    // Mobile
    if (!formData.mobile) {
      newErrors.mobile = t("supplier.mobileRequired");
    } else {
      const digitsOnly = formData.mobile.replace(/\D/g, "");
      if (digitsOnly.length < 6 || digitsOnly.length > 15) { 
        newErrors.mobile = t("supplier.mobileInvalid");
      }
    }

    // Email
    if (!formData.email) {
      newErrors.email = t("supplier.emailRequired");
    } else if (!/\S+@\S+\.\S+/.test(formData.email)) {
      newErrors.email = t("supplier.emailInvalid");
    }

    // Password
    if (!formData.password) {
      newErrors.password = t("supplier.passwordRequired");
    } else if (
      ...
      newErrors.password = t("supplier.passwordInvalid");
    }

    // Designation
    if (!formData.designation) {
      newErrors.designation = t("supplier.designationRequired");
    } else if (!/^[A-Za-z][A-Za-z\s]*$/.test(formData.designation.trim())) {
      newErrors.designation = t("supplier.designationInvalid");
    }
      ...
      <div className="card shadow-sm p-4"style={{backgroundColor: darkMode ? "#3d3d3dff" : "#ffff",color: darkMode ? "#e6eef8" : "#212529"}}>
        ...
        <div className="row mb-3">
          ...
          <div className="col-md-4">
            <label
              ...
              style={{ fontSize: "15px", fontWeight: 600 }}
            >
              {t("supplier.supid")} <span className="text-danger">*</span>
            </label>
            <input
              type="text"
              className={`form-control ${errors.lastName ? "is-invalid" : ""}`}
          ...
          <div className="col-md-4">
            <label
              ...
              style={{ fontSize: "15px", fontWeight: 600 }}
            >
              {t("supplier.zone")} <span className="text-danger">*</span>
            </label>
            <Select
              options={zoneOptions}
          ...
          >
            <label
              ...
              style={{ fontSize: "15px", fontWeight: 600 }}
            >
              {t("supplier.id")} <span className="text-danger">*</span>
            </label>
            <input
              type="text"
        ...
        <div className="row mb-3">
          <div className="col-md-4">
            <label
              ...
              style={{ fontSize: "15px", fontWeight: 600 }}
            >
              {t("supplier.firstname")} <span className="text-danger">*</span>
            </label>
            <input
              type="text"
          ...
          <div className="col-md-4">
            <label
              ...
              style={{ fontSize: "15px", fontWeight: 600 }}
            >
              {t("supplier.lastname")} <span className="text-danger">*</span>
            </label>
            <input
              type="text"
              className={`form-control ${errors.lastName ? "is-invalid" : ""}`}
        ...
        <div className="row mb-3">
          ...
          <div className="col-md-4">
            <label
              ...
              style={{ fontSize: "15px", fontWeight: 600 }}
            >
              {t("supplier.password")} <span className="text-danger">*</span>
            </label>
            <input
              type="text"
          ...
          <div className="col-md-4">
            <label
              ...
              style={{ fontSize: "15px", fontWeight: 600 }}
            >
              {t("supplier.designation")} <span className="text-danger">*</span>
            </label>
            <input
              type="text"
              className={`form-control ${
        ...
        <div className="d-flex gap-2 mt-3">
          ...
          <button className="btn btn-secondary" onClick={handleCancel}>
            <FaTimes className="me-1" /> {t("supplier.cancel")}
          </button>
        </div>
      </div>
    </div>
