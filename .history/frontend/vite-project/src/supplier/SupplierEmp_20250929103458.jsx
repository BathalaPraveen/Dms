...
import { FaPencilAlt, FaTrashAlt,FaRegEye, FaRegFilePdf, FaFileExcel,FaPlus } from "react-icons/fa";
import Table from "../components/Table";
import Delete from "../components/Delete";
import { useParams } from "react-router-dom";

const SupplierEmpList = () => {
  const navigate = useNavigate();
  const { t } = useTranslation();
  const { darkMode } = useTheme();
  const { index: supplierIndex } = useParams(); 
  ...
  const handleEdit = (index) => navigate(`/supplier/supplieremp/supempedit/${supplierIndex}/${index}`);
  const handleView = (index) => navigate(`/supplier/supplieremp/supplierview/${supplierIndex}/${index}`);
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor("employeeName", { header: t("supplier.staffName"), cell: (info) => info.getValue() }),
      columnHelper.accessor("employeeId", { header: t("supplier.staffId"), cell: (info) => info.getValue() }),
      columnHelper.accessor("employeeType", { header: t("supplier.staffType"), cell: (info) => info.getValue() }),
  ...
  const handleEmployeeAdd = () => navigate(`/supplier/supplieremp/supempadd/${supplierIndex}`);
    ...
    <div className="container mt-3">
      ...
      >
        <div className="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center">
          <h4 className="mb-2 mb-md-0">{t("supplier.supemptitle")}</h4>
