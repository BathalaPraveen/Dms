...
const Table = ({ data, columns, totalRecords, onPaginationChange, onFilteredRowsChange, collapsed }) => {
  ...
  const [expandedRows, setExpandedRows] = useState({}); // mobile accordion
  ...
  // Decide which column is the "main" column for mobile preview
  const mainColumn = columns.find(col => col.accessorKey === "employeeId") || columns[0];
  ...
  return (
    ...
    >
      ...
      {/* Mobile Accordion: Only mainColumn + toggle */}
      <div className="d-md-none">
        {table.getRowModel().rows.map(row => {
          const isExpanded = expandedRows[row.id];
          const mainCell = row.getVisibleCells().find(cell => cell.column.id === mainColumn.id);

          return (
            <div key={row.id} className={`card mb-2 ${darkMode ? "bg-dark text-white" : "bg-light text-dark"}`}>
              <div className="d-flex justify-content-between align-items-center p-2">
                <span>
                  <strong>{mainCell ? flexRender(mainColumn.header, mainCell.getContext()) : "N/A"}</strong>
                </span>
                <button
              ...
              {isExpanded && (
                <div className="p-2 border-top">
                  {row.getVisibleCells().map(cell => {
                    if (cell.column.id === mainColumn.id) return null; // skip main column
                    return (
                      <div key={cell.id} className="d-flex justify-content-between mb-1">
