  ...
  const renderMenuSection = (key, Icon, title, items) => (
    ...
    >
      ...
      {!collapsed && openMenu === key && (
        <ul className="nav flex-column border-start" style={{
      marginLeft: "12px",       // ✅ shift submenu a bit right
      paddingLeft: "10px",      // extra spacing inside submenu
      borderColor: "#cccccc",   // optional light border
    }}>
          {items.map((item) => (
            <li
              key={item}
