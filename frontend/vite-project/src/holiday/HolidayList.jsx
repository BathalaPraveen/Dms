...
const ApiTable = ({ collapsed }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const storedUsers = localStorage.getItem("holidayData"); // your key
      if (storedUsers) {
  ...
  const handleDelete = (index) => {
    Delete({
      ...
      onConfirm: () => {
        ...
        localStorage.setItem("holidayData", JSON.stringify(updatedUsers));
      },
    });
  };
  ...
  const columns = useMemo(
    () => [
      columnHelper.accessor("state", { header: t("holiday.state"), cell: (info) => info.getValue() }),
      ...
      columnHelper.accessor("holidayInfo", { header: t("holiday.info"), cell: (info) => info.getValue() }),
  ...
  const exportPdf = () => {
    ...
    const data = filteredRows.map(row =>
      row.getVisibleCells().filter(cell => cell.column.id !== "actions").map(cell => {
        ...
        let value = cell.row.original[accessor];

        // Format date field
        if (accessor === "holidayDate" && value) {
    ...
    generatePdf(t("holiday.holidayrep"), headers, data);
  };
  ...
  const exportExcel = () => {
    ...
    exportToExcel(t("holiday.holidayrep"), headers, data);
  };
  ...
  return (
    <div className="container">
      <div className='card mb-2 p-3' style={{backgroundColor: darkMode ? "#3d3d3dff" : "#ffff",color: darkMode ? "#e6eef8" : "#212529"}}>
        ...
        <h4 className="mb-2 mb-md-0">{t("holiday.holidaylist")}</h4>
          <div className="d-flex flex-wrap gap-2">
            ...
            <button className="btn btn-primary" onClick={() => navigate("/holiday/holidayadd")}><FaPlus /></button>
          </div>
        </div>
      </div>
